...
version(Windows) {
    ...
    @safe nothrow @nogc:
        ...
        bool triggerRead(SocketState* socketState, bool tryToFulfill = true) scope @trusted {
            ...
            bool ret, needFulFill;
            socketState.rawReadingState.protectTriggeringOfRead(() @trusted {
                const uint bufferNeeded = cast(uint)socketState.encryptionState.amountNeedToBeRead(socketState);
                assert(bufferNeeded > 0);
                socketState.rawReadingState.prepareBufferFor(bufferNeeded);
                ...
                if(result == 0) {
                    ...
                    logger.debug_("Triggering read without delay ", this.handle, " for amount ", received, " with flags ", flags);
                    socketState.rawReadingState.dataWasReceivedAlreadyProtected(received);
                    ret = true;
                    needFulFill = true;
                    return false;
                } else if(result == SOCKET_ERROR) {
                    ...
                    if(error == ERROR_IO_PENDING) {
                        logger.debug_("Triggering read with delay in IOCP ", this.handle, " with amount ", bufferNeeded, " for flags ", flags, " ", received);
            ...
            if(needFulFill)
                return socketState.readingState.tryFulfillRequest(socketState);
        ...
        bool triggerWrite(scope SocketState* socketState) scope @trusted {
            ...
            return socketState.rawWritingState.protect(() @trusted nothrow @nogc {
                ...
                while(socketState.rawWritingState.toSend.length > 0 && socketState.rawWritingState.waitingOnDataToSend == 0) {
                    ...
                    if(result == 0) {
                        // ok sent
                        logger.debug_("Socket has had data written to it ", this.handle, " for items ", amountSent);
                        socketState.rawWritingState.complete(amountSent);
                        didSomething = true;
                    } else {
                        auto error = GetLastError();
